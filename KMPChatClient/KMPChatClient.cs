...
namespace KMPChatClient
{
	class ChatClient
	{
		...
		static bool receiveThreadRunning = false;
		static bool sendThreadRunning = false;
		static bool chatLoopRunning = false;
		static bool quitAllThreads = false;
		//static string data;
			...
			if (address != null) {
				...
				try {
					...
					if (chatTCPSocket.Connected) {
						...
						receiveThread = new Thread (new ThreadStart (handleConnection));
		...
		static void handleConnection ()
		{
			...
			while (receiveThreadRunning && !quitAllThreads) {
				//Get the header
				...
				if (header_received == true) {
					...
					while (received_message_index < message_size) {
						...
						if (received_message_index == message_size) {
							byte[] decompressed_received_message = Decompress (received_message);
							receieveMessage ((ServerMessageID)message_id, decompressed_received_message);
		...
		static void receieveMessage (ServerMessageID messageID, byte[] message_data)
		{
			...
			case ServerMessageID.HANDSHAKE:
				...
				if (server_net_protocol >= MIN_NET_PROTOCOL && server_net_protocol <= MAX_NET_PROTOCOL) {
					...
					quitAllThreads = true;
					Console.WriteLine("Chat client is not compatible with server");
				}
				break;
			...
			case ServerMessageID.SCENARIO_UPDATE:
				printIfDebug ("Unhandled Message: SCENARIO_UPDATE");
				break;
			...
			case ServerMessageID.CONNECTION_END:
				Console.WriteLine ("CONNECTION_END:" + Encoding.UTF8.GetString (message_data));
				receiveThreadRunning = false;
				sendThreadRunning = false;
				break;
